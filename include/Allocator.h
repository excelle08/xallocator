...
class Allocator {
 public:
  ...
  /// @param[in]	memory - pointer to a block of static memory for allocator or
  /// NULL
  ///		to obtain memory from global heap. If not NULL, the objects
  ///argument 		defines the size of the memory block (size x objects = memory size
  ///in bytes).
  ///	@param[in]	name - optional allocator name string.
  Allocator(size_t size, UINT objects = 0, CHAR* memory = NULL,
  ...
  /// @return		A pointer to the allocator name or NULL if none was
  /// assigned.
...
#define DECLARE_ALLOCATOR                                                 \
 public:                                                                  \
  void* operator new(size_t size) { return _allocator.Allocate(size); }   \
  void operator delete(void* pObject) { _allocator.Deallocate(pObject); } \
                                                                          \
 private:                                                                 \
