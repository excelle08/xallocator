...
#define POOL_SIZE                                 \
  DATA_CAP * 1024 + sizeof(Allocator) * NBLOCKS + \
      sizeof(PoolListNode) * NBLOCKS + sizeof(PoolListHeader) * NHEADERS
...
static void lock_destroy() { _xallocInitialized = FALSE; }
...
Allocator *new_secondary_allocator(size_t size) {
  ...
  Allocator *sec_allocator =
      static_cast<Allocator *>(my->secondaries.Allocate(sizeof(Allocator)));
  ...
  new (sec_allocator)
      Allocator(size, my->blocksize / size, pool, "secondary allocator");
  return sec_allocator;
}

PoolListNode *new_secondary_node(size_t size, PoolListNode **nodeptr) {
  PoolListNode *newnode = static_cast<PoolListNode *>(
      my->secondary_nodes.Allocate(sizeof(PoolListNode)));
...
Allocator *find_allocator(size_t size) {
  ...
  if (size > my->blocksize / 2) return &my->primary;
...
#else
extern "C" void xalloc_init(size_t data_capacity, size_t blocksize,
                            void *user_pool) {
  lock_init();
  memory = user_pool;
  ...
  new (&my->primary)
      Allocator(blocksize, nblocks, datapool, "Primary allocator");
  /* Secondaries */
  char *secondary_pool = datapool + data_capacity;
  new (&my->secondaries) Allocator(sizeof(Allocator), nblocks, secondary_pool,
                                   "Allocator of secondary allocators");
  char *sec_nodes_pool = secondary_pool + sizeof(Allocator) * nblocks;
  new (&my->secondary_nodes)
      Allocator(sizeof(PoolListNode), nblocks, sec_nodes_pool,
                "Allocators for secondary list nodes");
  my->n_headers = BITS_TO_REPRESENT(blocksize) - 4;
  my->secondary_headers =
      new (sec_nodes_pool + sizeof(PoolListNode) * nblocks) PoolListHeader();
  for (int i = 0; i < my->n_headers; ++i) {
...
extern "C" void *xmalloc(size_t size) {
  ...
  if (blockMemoryPtr == nullptr) return nullptr;
...
extern "C" void *xrealloc(void *oldMem, size_t size) {
  ...
  if (size + sizeof(Allocator *) <= this_block_size &&
      size + sizeof(Allocator *) > this_block_size / 2)
    return oldMem;
